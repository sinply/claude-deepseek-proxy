...
`proxy-watchdog.ps1` 作为隐藏后台进程运行（通过 `install-autostart.ps1` 安装为登录计划任务）。每 5 秒轮询一次 `Claude.exe`：
...
`proxy-config.json`（与 `model-rewrite-proxy.cjs` 放在同一目录）定义 provider：
...
要修改某个 Claude 名对应的真实模型，编辑 `proxy-config.json`：

1. 打开 `proxy-config.json`。
...
3. 保存文件。
4. 重启代理以加载新配置：
   - 如果看门狗正在运行：结束代理进程，看门狗会在几秒内重新拉起它。
   - 如果手动运行：结束当前 `node` 进程，然后重新运行 `start-claude-deepseek-proxy.ps1`。
...
- `model-rewrite-proxy.cjs` — 代理实现：按 provider 路由、模型名重写、`/v1/models` 拦截。
- `proxy-config.json` — provider 定义（上游 URL + 模型映射）。
- `proxy-watchdog.ps1` — 后台看门狗，同步代理与 Claude Code 的生命周期。
- `install-autostart.ps1` / `uninstall-autostart.ps1` — 安装/卸载看门狗计划任务。
- `start-claude-deepseek-proxy.ps1` — 独立启动代理（无看门狗），保留作手动使用。
- `start-claude.ps1` — 另一种包装脚本：启动代理 + Claude，并在 Claude 退出时停止代理。
- `status.ps1` — 查看计划任务 / 看门狗 / 代理 / Claude 状态。
...
- 日志：`watchdog.log` 记录看门狗动作，每次代理启动会生成一个 `proxy-<timestamp>.log`。所有日志文件已被 Git 忽略。
- 不要向 Git 提交私钥、Claude Code 配置文件、`proxy-*.log` 或 `watchdog.log`。
